...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!departmentSchedules || !staffList) {
      return res.status(400).json({ error: 'Thiếu thông tin phân tích dữ liệu.' });
    }
    ...
    try {
      ...
      if (aiResponseText) {
        return res.status(200).json({ analysis: aiResponseText });
      } else {
      ...
      return res.status(200).json({ analysis: fallbackReport });
    }
  } catch (error: any) {
    const errorMsg = error?.message || String(error);
    console.warn('[NurseScheduleAdvisor-General] Routing issue in analyze-schedule handler:', errorMsg);
    return res.status(500).json({ error: errorMsg || 'Lỗi hệ thống ngoài tầm kiểm soát khi phân tích lịch.' });
  }
}
